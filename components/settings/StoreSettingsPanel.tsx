...
/* ─── Panel ──────────────────────────────────────────────────────────── */
// Reusable across the owner's /stores/[id]/settings route and the admin panel's
// store picker — takes a storeId directly instead of reading it from the URL.
export default function StoreSettingsPanel({ storeId, showHeader = true }: { storeId: string; showHeader?: boolean }) {
  const { store, loading, refresh } = useStore(storeId);
